...
export class Leetify {
  ...
  login() {
    return axios('https://api.leetify.com/api/login', {
      ...
      data: {
        email: process.env.LEETIFY_USER,
        password: process.env.LEETIFY_PASSWORD,
      },
      method: 'POST',
  ...
  getClubLeaderboard() {
    return this.fetcher(
      `https://api.leetify.com/api/club/${config.leetify.clubId}/member-comparison/impact?gameVersions=cs2`
    );
  }

  getClubSessions() {
    return this.fetcher(
      `https://api.leetify.com/api/sessions/${config.leetify.clubId}`
    ).then((res) => {
