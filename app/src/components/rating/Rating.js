    ...
    <ResponsiveContainer width="100%" height={500}>
      <LineChart data={rating}>
        {rating[0].rating.map((line, index) => (
          <Line type="monotone" name={line.user} dataKey={(data) => data.rating[index].rank} stroke="#8884d8" strokeWidth={3} />
        ))}
        <CartesianGrid stroke="#ccc" />
        <XAxis dataKey={(data) => data.date} tickFormatter={(tick) => new Date(tick).toLocaleDateString()} />
        <YAxis />
        <Legend />
      </LineChart>
    </ResponsiveContainer>
