...
export const katkuHandler = async (msg) => {
    ...
    const timeQuestionMessage = await bot.sendMessage(chatId, 'О котрій годині?', { reply_to_message_id: msg.message_id });
    ...
    const replyTimeId = bot.onReplyToMessage(chatId, timeQuestionMessage.message_id, async (replyTimeMsg) => {
        ...
        if (!time || !time.match(/^\d\d?[:\-\s]\d\d$/)) {
            return bot.sendMessage(chatId, 'Ти що даун? Вкажи нормально час. 21:30, наприклад');
        }

        const [hours, minutes] = time.split(':');
        const targetDate = new Date().setHours(hours, minutes);

        const pollMsg = await bot.sendPoll(chatId,`Сьогодні граєм в ${time}?`, ['Так', 'Можливо', 'Буду пізніше', 'Я хуйня підзалупна'] , { is_anonymous: false });
        ...
        const interval = setInterval(async () => {
            ...
            bot.editMessageText(`До кінця голосування: ${leftTimeText}`, { message_id: timerMsg.message_id, chat_id: chatId });
